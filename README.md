...
Ниже пример, как это делаю я (через conda).
...
### 3) (Опционально) установить инструменты для тестов
...
`vllm` не перечислен в `pyproject.toml`, его нужно поставить отдельно.
...
- **vLLM не стартует**: проверьте соответствие `CUDA_VISIBLE_DEVICES` и `--tensor-parallel-size` количеству доступных GPU.
- **Нет результатов full-text**: это нормально для части статей — система умеет продолжать работу по abstract.
- **Ошибки Entrez/PubMed**: проверьте интернет и корректность `ENTREZ_EMAIL`.
- **Проблемы с моделью**: убедитесь, что `VLLM_BASE_URL`, `VLLM_API_KEY` и имя модели совпадают с конфигурацией запущенного vLLM.
